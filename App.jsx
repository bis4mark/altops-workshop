...
const ESP="#2b2019",OAK="#b0894e",CREAM="#f7f2e8",CARD="#fffdf8",INK="#2a221b",INK2="#7a6f62",LINE="#e4dac6";
...
const WOOD_TYPES=[{name:"Mahogany",pricePerSheet:420},{name:"Odum (Iroko)",pricePerSheet:380},{name:"Wawa",pricePerSheet:220},{name:"Sapele",pricePerSheet:460},{name:"MDF Board",pricePerSheet:180},{name:"Plywood",pricePerSheet:150}];
const CABINET_TYPES=[
...
async function loadJobs(){if(!hasStore)return[];try{const out=[];for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith("wjob_")){try{out.push(JSON.parse(localStorage.getItem(k)));}catch(e){}}}return out;}catch(e){return[];}}
...
/* image helpers */
function fileToThumb(file,max=1100,q=0.78){return new Promise((res,rej)=>{const img=new Image();const url=URL.createObjectURL(file);img.onload=()=>{const s=Math.min(1,max/Math.max(img.width,img.height));const w=Math.round(img.width*s),h=Math.round(img.height*s);const c=document.createElement("canvas");c.width=w;c.height=h;c.getContext("2d").drawImage(img,0,0,w,h);URL.revokeObjectURL(url);res(c.toDataURL("image/jpeg",q));};img.onerror=rej;img.src=url;});}
function canvasDown(srcCanvas,max=1100,q=0.8){const s=Math.min(1,max/Math.max(srcCanvas.width,srcCanvas.height));const w=Math.round(srcCanvas.width*s),h=Math.round(srcCanvas.height*s);const c=document.createElement("canvas");c.width=w;c.height=h;c.getContext("2d").drawImage(srcCanvas,0,0,w,h);return c.toDataURL("image/jpeg",q);}

/* plan engine */
function carcassParts(W,H,D,t,shelves,doors){const iw=W-2*t,p=[];const add=(name,mat,L,Wd,qty)=>{if(qty>0&&L>0&&Wd>0)p.push({name,mat,L:Math.round(L),Wd:Math.round(Wd),qty});};add("Side panel","Carcass",D,H,2);add("Top / bottom","Carcass",iw,D,2);if(shelves>0)add("Shelf","Carcass",iw,D-20,shelves);if(doors>0)add("Door","Carcass",H-4,W/doors-3,doors);add("Back panel","Back",W,H,1);return p;}
function tableParts(L,W,H,t){const p=[];const add=(name,mat,a,b,qty)=>p.push({name,mat,L:Math.round(a),Wd:Math.round(b),qty});add("Top","Carcass",L,W,1);add("Leg","Carcass",H-t,89,4);add("Apron (long)","Carcass",L-178,89,2);add("Apron (short)","Carcass",W-178,89,2);return p;}
function packMaterial(pieces,SW,SH,kerf){const norm=pieces.map((p)=>{let w=p.L,h=p.Wd;if(h>w){const t=w;w=h;h=t;}return{...p,w,h};});norm.sort((a,b)=>b.h-a.h||b.w-a.w);const sheets=[];for(const piece of norm){if(piece.w>SW||piece.h>SH){if(piece.h<=SW&&piece.w<=SH){const t=piece.w;piece.w=piece.h;piece.h=t;}}let placed=false;for(const sheet of sheets){for(const shelf of sheet.shelves){const gap=shelf.x>0?kerf:0;const orient=[[piece.w,piece.h],[piece.h,piece.w]];let ch=null;for(const[ow,oh]of orient)if(shelf.x+gap+ow<=SW&&oh<=shelf.height){ch=[ow,oh];break;}if(ch){shelf.x+=gap+ch[0];placed=true;break;}}if(placed)break;const nextY=sheet.bottom>0?sheet.bottom+kerf:0;if(nextY+piece.h<=SH){sheet.shelves.push({x:piece.w,height:piece.h});sheet.bottom=nextY+piece.h;placed=true;break;}}if(!placed){sheets.push({shelves:[{x:piece.w,height:piece.h}],bottom:piece.h});}}return sheets.length;}
function buildPlan(job,t,shelves,doors){const W=+job.W,H=+job.H,D=+job.D;if(!W||!H)return null;const parts=job.type==="Table"?tableParts(W,D||600,H,t):carcassParts(W,H,D||570,t,shelves,doors);const groups={};for(const p of parts){(groups[p.mat]||=[]);for(let i=0;i<p.qty;i++)groups[p.mat].push({L:p.L,Wd:p.Wd});}const boards=Object.entries(groups).map(([mat,pcs])=>({mat,count:packMaterial(pcs,2440,1220,3)}));return{parts,boards,t,shelves,doors,at:Date.now()};}

/* flex/grid helpers replacing Tailwind */
...
const grid2=(extra={})=>({display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12,...extra});
const grid4=(extra={})=>({display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,...extra});
const grid3=(extra={})=>({display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12,...extra});
const wrap=(extra={})=>({display:"flex",flexWrap:"wrap",alignItems:"center",...extra});
...
export default function AltopsWorkshop(){
  ...
  useEffect(()=>{(async()=>{const j=await loadJobs();const s=await loadSettings();setJobs(j.sort((a,b)=>b.createdAt-a.createdAt));if(s)setSettings(s);setLoading(false);})();},[]);

  const upsert=(job)=>setJobs((js)=>{const i=js.findIndex((x)=>x.id===job.id);return(i>=0?js.map((x)=>(x.id===job.id?job:x)):[job,...js]).sort((a,b)=>b.createdAt-a.createdAt);});
  ...
  const addSample=()=>saveJob({...emptyJob(),title:"Reception counter unit",client:"Shop fit-out",type:"Counter / Desk",status:"done",W:"1500",H:"1100",D:"600",material:"18mm melamine + plywood carcass",price:"3200",deposit:"1500",portfolio:true,blurb:"Two-tier reception counter with raised transaction top and lockable base cabinet."});

  const stats={total:jobs.length,active:jobs.filter((j)=>j.status==="progress").length,value:jobs.reduce((a,j)=>a+(+j.price||0),0),outstanding:jobs.filter((j)=>j.status!=="delivered").reduce((a,j)=>a+Math.max(0,(+j.price||0)-(+j.deposit||0)),0)};
  ...
  return(
    <div style={{minHeight:"100vh",background:CREAM,color:INK,fontFamily:"ui-sans-serif,system-ui,sans-serif"}}>
      ...
      <div style={{background:ESP,color:CREAM}}>
        <div style={{maxWidth:1100,margin:"0 auto",padding:"12px 16px",...row({justifyContent:"space-between",flexWrap:"wrap",gap:8})}}>
          <div style={row({gap:8})}><span style={{width:12,height:12,background:OAK,borderRadius:2,display:"inline-block"}}/><span style={{fontFamily:"Georgia,serif",fontWeight:700,fontSize:18}}>{settings.business}</span></div>
          <div style={row({gap:8})}>
            <div style={{display:"flex",background:"#3b2d22",borderRadius:8,padding:2}}>
              {[["workshop","Workshop"],["portfolio","Portfolio"]].map(([k,l])=>(
                <button key={k} onClick={()=>{setEditing(null);setView(k);}} style={{padding:"6px 14px",borderRadius:6,fontSize:14,fontWeight:600,background:view===k?OAK:"transparent",color:view===k?ESP:"#d8cbb8",border:"none",cursor:"pointer"}}>{l}</button>
              ))}
            </div>
            <button onClick={()=>setShowSettings(true)} style={{padding:"6px 10px",borderRadius:6,background:"#3b2d22",color:"#d8cbb8",border:"none",cursor:"pointer"}}>⚙</button>
          </div>
        </div>
      </div>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 16px"}}>
        {editing?(
          ...
          <>
            <div style={{...grid4(),marginBottom:18}}>
            ...
            <div style={{...wrap({gap:8}),marginBottom:14}}>
              <button onClick={()=>setEditing(emptyJob())} style={{background:ESP,color:CREAM,padding:"9px 16px",borderRadius:8,fontWeight:600,fontSize:14,border:"none",cursor:"pointer"}}>+ New job</button>
            ...
            {visible.length===0?<EmptyState onNew={()=>setEditing(emptyJob())} onSample={addSample}/>:(
              <div style={grid3()}>{visible.map((j)=><JobCard key={j.id} job={j} onOpen={()=>setEditing(j)}/>)}</div>
            )}
          </>
        ):(
...
function Dot({on,label}){return <span style={{display:"inline-flex",alignItems:"center",gap:3,fontSize:10,color:on?"#4f9d69":"#c3b596"}}><span style={{width:7,height:7,borderRadius:999,background:on?"#4f9d69":"#d8cdb5",display:"inline-block"}}/>{label}</span>;}

function JobCard({job,onOpen}){
  const cover=job.photos&&job.photos[0];const bal=Math.max(0,(+job.price||0)-(+job.deposit||0));
  return(
    <button onClick={onOpen} style={{textAlign:"left",background:CARD,border:`1px solid ${LINE}`,borderRadius:12,overflow:"hidden",cursor:"pointer",padding:0,width:"100%"}}>
      ...
      <div style={{padding:12}}>
        <div style={row({justifyContent:"space-between",gap:4})}><Badge s={job.status}/>{job.portfolio&&<span style={{fontSize:10,color:OAK,fontWeight:700}}>★ PORTFOLIO</span>}</div>
        ...
        <div style={{...row({gap:8}),marginTop:8}}><Dot on={(job.measurements||[]).length>0} label="Measured"/><Dot on={!!job.estimate} label="Estimated"/><Dot on={!!job.plan} label="Planned"/></div>
        <div style={{...row({justifyContent:"space-between"}),marginTop:8,fontSize:12}}><span style={{fontFamily:"ui-monospace,monospace",color:INK}}>{money(job.price)}</span>{bal>0&&<span style={{color:"#c05c4d"}}>bal {money(bal)}</span>}</div>
      </div>
    </button>
  );
}
...
function EmptyState({onNew,onSample}){
  return <div style={{textAlign:"center",padding:"48px 16px",background:CARD,border:`1px dashed ${LINE}`,borderRadius:12}}>
    ...
    <div style={{...row({justifyContent:"center",gap:8}),marginTop:14}}>
      <button onClick={onNew} style={{background:ESP,color:CREAM,padding:"9px 16px",borderRadius:8,fontWeight:600,border:"none",cursor:"pointer"}}>+ New job</button>
      <button onClick={onSample} style={{background:"transparent",color:OAK,padding:"9px 16px",borderRadius:8,fontWeight:600,border:`1px solid ${OAK}`,cursor:"pointer"}}>Add sample project</button>
    </div></div>;
}

function Field({label,children,full}){return <label style={{...col({gap:4}),gridColumn:full?"1 / -1":"auto"}}><span style={{fontSize:12,color:INK2}}>{label}</span>{children}</label>;}
const inp={padding:"8px 10px",borderRadius:8,border:`1px solid ${LINE}`,background:CARD,color:INK,fontSize:14,width:"100%",boxSizing:"border-box"};

function JobEditor({job,onSave,onPatch,onCancel,onDelete,setLightbox,settings}){
  const[j,setJ]=useState(job);const[tab,setTab]=useState("details");
  ...
  return(
    <div style={{background:CARD,border:`1px solid ${LINE}`,borderRadius:12,padding:18}}>
      <div style={{...row({justifyContent:"space-between"}),marginBottom:12}}>
        ...
        <button onClick={onCancel} style={{color:INK2,background:"none",border:"none",cursor:"pointer"}}>✕ Close</button>
      </div>
      <div style={{...wrap({gap:4}),marginBottom:16,borderBottom:`1px solid ${LINE}`,paddingBottom:8}} className="no-print">
        {TABS.map(([k,l])=>{const done=k==="measure"?(j.measurements||[]).length>0:k==="estimate"?!!j.estimate:k==="plan"?!!j.plan:false;
          return <button key={k} onClick={()=>setTab(k)} style={{padding:"6px 12px",borderRadius:8,fontSize:13,fontWeight:600,background:tab===k?ESP:"transparent",color:tab===k?CREAM:INK,border:"none",cursor:"pointer"}}>{l}{done&&tab!==k?" ✓":""}</button>;
        })}
      </div>
      {tab==="details"&&<DetailsPanel j={j} up={up}/>}
      {tab==="measure"&&<MeasurePanel j={j} patch={patch} setLightbox={setLightbox} onUseDims={(m)=>{patch({W:String(Math.round(m.length_cm*10)),H:String(Math.round(m.width_cm*10))});setTab("details");}}/>}
      ...
      <div style={{...row({justifyContent:"space-between"}),marginTop:18,borderTop:`1px solid ${LINE}`,paddingTop:14}} className="no-print">
        <button onClick={()=>onDelete(j.id)} style={{color:"#c05c4d",fontSize:14,background:"none",border:"none",cursor:"pointer"}}>Delete job</button>
        <div style={row({gap:8})}>
          <button onClick={onCancel} style={{padding:"9px 16px",borderRadius:8,border:`1px solid ${LINE}`,color:INK,background:CARD,cursor:"pointer"}}>Cancel</button>
          <button onClick={()=>onSave(j)} style={{padding:"9px 18px",borderRadius:8,background:ESP,color:CREAM,fontWeight:600,border:"none",cursor:"pointer"}}>Save job</button>
        </div>
      </div>
    </div>
  );
...
function DetailsPanel({j,up}){
  return(
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12}}>
      <Field label="Job title" full><input style={inp} value={j.title} onChange={(e)=>up("title",e.target.value)} placeholder="e.g. Reception counter"/></Field>
      ...
      <div style={{gridColumn:"1 / -1",padding:12,background:"#faf6ec",border:`1px solid ${LINE}`,borderRadius:10}}>
        <label style={{...row({gap:8}),cursor:"pointer"}}><input type="checkbox" checked={j.portfolio} onChange={(e)=>up("portfolio",e.target.checked)}/><span style={{fontWeight:600,color:INK}}>Show this build in my public portfolio</span></label>
...
function MeasurePanel({j,patch,setLightbox,onUseDims}){
  const[ref,setRef]=useState(REF_OBJECTS[0]);const[customRef,setCustomRef]=useState("");
  const[busy,setBusy]=useState(false);const[err,setErr]=useState(null);const[cam,setCam]=useState(false);
  ...
  const analyze=async(dataUrl)=>{
    ...
    try{
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1000,messages:[{role:"user",content:[
          ...
          {type:"text",text:`You measure wood for a furniture workshop. Reference object in image: ${refText}. Respond ONLY with JSON (no markdown): {"items":[{"label":"Board 1","length_cm":190,"width_cm":60,"thickness_cm":1.8,"material_guess":"MDF","condition":"new"}],"total_boards":1,"reference_detected":true,"confidence":"high","recommended_product":"cabinet panels"}`}
        ]}]})});
    ...
    }catch(e){setErr("Could not analyze the photo. Try better lighting and a clearly visible reference object.");}
    setBusy(false);
  };
  ...
  const onUpload=async(f)=>{
    ...
    if(!allowed.includes(f.type)){setErr(`Please upload a JPEG, PNG or WEBP photo. You selected "${f.type||"unknown format"}". If your photo is HEIC, take a screenshot of it first then upload that.`);return;}
    try{const url=await fileToThumb(f,1100,0.8);analyze(url);}catch(e){setErr("Could not read that image. Please use a JPEG, PNG or WEBP photo.");}
  };
  return(
    <div>
      {!cam?(
        <>
          <div style={{background:"#faf6ec",border:`1px solid ${LINE}`,borderRadius:10,padding:14,marginBottom:12}}>
            <div style={{fontSize:12,color:INK2,marginBottom:8}}>Place a reference object beside the wood so AI can estimate real size.</div>
            <div style={wrap({gap:6})}>{REF_OBJECTS.map((r)=><button key={r} onClick={()=>setRef(r)} style={{fontSize:12,padding:"6px 10px",borderRadius:8,border:`1px solid ${ref===r?OAK:LINE}`,background:ref===r?"#f2e6cc":CARD,color:INK,cursor:"pointer"}}>{r}</button>)}</div>
          ...
          <div style={{...row({gap:8}),marginBottom:12}}>
            <button onClick={startCam} disabled={busy} style={{flex:1,padding:12,background:ESP,color:CREAM,borderRadius:8,fontWeight:600,border:"none",cursor:"pointer"}}>Open camera</button>
            <button onClick={()=>fileRef.current&&fileRef.current.click()} disabled={busy} style={{flex:1,padding:12,background:"transparent",color:OAK,border:`1px solid ${OAK}`,borderRadius:8,fontWeight:600,cursor:"pointer"}}>Upload photo (JPEG/PNG)</button>
            <input ref={fileRef} type="file" accept="image/jpeg,image/jpg,image/png,image/webp" style={{display:"none"}} onChange={(e)=>onUpload(e.target.files&&e.target.files[0])}/>
          </div>
        </>
      ):(
        <div style={{position:"relative",borderRadius:10,overflow:"hidden",background:"#000",marginBottom:12}}>
          ...
          <div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(transparent,rgba(0,0,0,0.8))",padding:14,...row({justifyContent:"space-between",alignItems:"center"})}}>
            <button onClick={stopCam} style={{background:"rgba(255,255,255,0.15)",color:"#fff",padding:"8px 14px",borderRadius:6,border:"none",cursor:"pointer"}}>Cancel</button>
            <button onClick={capture} style={{width:62,height:62,borderRadius:"50%",background:OAK,border:"4px solid #fff",cursor:"pointer"}}/>
            <span style={{width:60}}/>
          </div>
        </div>
      )}
      ...
      {(j.measurements||[]).map((m)=>(
        <div key={m.id} style={{background:CARD,border:`1px solid ${LINE}`,borderRadius:10,padding:12,marginBottom:10}}>
          <div style={row({gap:12})}>
            <img src={m.image} onClick={()=>setLightbox(m.image)} alt="" style={{width:84,height:84,objectFit:"cover",borderRadius:8,cursor:"pointer",flexShrink:0}}/>
            <div style={{flex:1}}>
              <div style={row({justifyContent:"space-between"})}><span style={{fontSize:12,fontWeight:700,color:INK}}>{m.result.total_boards} board(s) · {m.result.confidence} confidence</span><span style={{fontSize:11,color:INK2}}>{m.date}</span></div>
              {(m.result.items||[]).map((it,i)=>(
                <div key={i} style={{marginTop:6,fontSize:13}}>
                  ...
                  <button onClick={()=>onUseDims(it)} style={{marginTop:4,fontSize:11,color:OAK,fontWeight:700,background:"none",border:"none",cursor:"pointer",padding:0}}>Use as job dimensions →</button>
                </div>
              ))}
...
function EstimatePanel({j,patch,settings}){
  ...
  const[cab,setCab]=useState(initCab);const[qty,setQty]=useState(seed?seed.qty:1);
  ...
  const[tt,setTt]=useState(seed?seed.tt:500);const[labour,setLabour]=useState(seed?seed.labour:9000);
  const[advance,setAdvance]=useState(seed?seed.advance:(+j.deposit||0));const[invoice,setInvoice]=useState(false);
  ...
  const woodCost=(+sheets)*wood.pricePerSheet;const subtotal=woodCost+(+machine)+(+fittings)+(+marble)+(+tt)+(+labour);const balance=subtotal-(+advance);
  ...
  return(
    <div>
      ...
      <div style={{background:"#faf6ec",border:`1px solid ${OAK}`,borderRadius:10,padding:14,marginTop:14}}>
        <div style={{...row({justifyContent:"space-between"}),fontSize:14,color:INK}}><span>Wood material</span><span>{money(woodCost)}</span></div>
        <div style={{...row({justifyContent:"space-between"}),fontSize:14,color:INK,marginTop:4}}><span>Other charges</span><span>{money(subtotal-woodCost)}</span></div>
        <div style={{...row({justifyContent:"space-between"}),borderTop:`1px solid ${OAK}`,marginTop:8,paddingTop:8,fontWeight:800,color:INK,fontFamily:"Georgia,serif",fontSize:18}}><span>Grand total</span><span>{money(subtotal)}</span></div>
        {(+advance)>0&&<div style={{...row({justifyContent:"space-between"}),fontSize:14,marginTop:4,color:balance<0?"#c05c4d":"#4f9d69"}}><span>Balance B/Forward</span><span>{money(balance)}</span></div>}
      </div>
      <div style={{...wrap({gap:8}),marginTop:12}} className="no-print">
        <button onClick={applyToJob} style={{padding:"9px 16px",background:ESP,color:CREAM,borderRadius:8,fontWeight:600,border:"none",cursor:"pointer"}}>Apply price to job</button>
        <button onClick={saveEstimate} style={{padding:"9px 16px",background:"transparent",border:`1px solid ${OAK}`,color:OAK,borderRadius:8,fontWeight:600,cursor:"pointer"}}>Save estimate</button>
        <button onClick={()=>setInvoice((v)=>!v)} style={{padding:"9px 16px",background:OAK,color:ESP,borderRadius:8,fontWeight:700,border:"none",cursor:"pointer"}}>{invoice?"Hide invoice":"Generate invoice"}</button>
      </div>
      {invoice&&(
        <div style={{background:"#fff",borderRadius:8,padding:"28px 24px",color:"#111",marginTop:14,border:`1px solid ${LINE}`}}>
          ...
          <div style={{marginTop:12,fontSize:13}}>
            <div style={row({justifyContent:"space-between"})}><span>Advance/paid</span><span>₵{Number(advance).toLocaleString("en-GH",{minimumFractionDigits:2})}</span></div>
            <div style={{...row({justifyContent:"space-between"}),fontWeight:"bold",marginTop:4}}><span>Balance B/Forward</span><span>₵{balance.toLocaleString("en-GH",{minimumFractionDigits:2})}</span></div>
          </div>
          ...
          <div style={{textAlign:"center",marginTop:16}} className="no-print"><button onClick={()=>window.print()} style={{padding:"10px 24px",background:ESP,color:OAK,borderRadius:6,border:"none",cursor:"pointer"}}>Print / Save as PDF</button></div>
        </div>
      )}
    </div>
  );
...
function PlanPanel({j,patch}){
  const[t,setT]=useState(j.plan?j.plan.t:18);const[shelves,setShelves]=useState(j.plan?j.plan.shelves:1);const[doors,setDoors]=useState(j.plan?j.plan.doors:2);
  const plan=j.plan;
  const generate=()=>{const p=buildPlan(j,+t,+shelves,+doors);if(p)patch({plan:p});};
  return(
    <div>
      {(!j.W||!j.H)&&<div style={{background:"#fbecec",color:"#9c3b30",borderRadius:8,padding:12,fontSize:13,marginBottom:12}}>Set Width and Height on the Details tab first.</div>}
      ...
      <button onClick={generate} disabled={!j.W||!j.H} style={{marginTop:12,padding:"9px 16px",background:ESP,color:CREAM,borderRadius:8,fontWeight:600,border:"none",cursor:"pointer",opacity:(!j.W||!j.H)?0.5:1}}>Generate cut plan</button>
      {plan&&(
        <div style={{marginTop:14}}>
          <div style={{...wrap({gap:8}),marginBottom:12}}>
          ...
          <div style={{marginTop:10}} className="no-print"><button onClick={()=>window.print()} style={{padding:"8px 16px",background:OAK,color:ESP,borderRadius:8,fontWeight:700,border:"none",cursor:"pointer"}}>Print cut plan</button></div>
        </div>
      )}
    </div>
  );
...
function PhotosPanel({j,setJ,setLightbox}){
  ...
  const add=async(files)=>{setBusy(true);const arr=[...(j.photos||[])];for(const f of Array.from(files).slice(0,8-arr.length)){try{arr.push(await fileToThumb(f,1000,0.72));}catch(e){}}setJ((x)=>({...x,photos:arr}));setBusy(false);};
  return(
    <div>
      <div style={{...row({justifyContent:"space-between"}),marginBottom:10}}>
        ...
        <button onClick={()=>fileRef.current&&fileRef.current.click()} disabled={busy} style={{fontSize:13,color:OAK,fontWeight:600,background:"none",border:"none",cursor:"pointer"}}>{busy?"Adding…":"+ Add photos"}</button>
        <input ref={fileRef} type="file" accept="image/*" multiple style={{display:"none"}} onChange={(e)=>e.target.files&&add(e.target.files)}/>
      </div>
      <div style={wrap({gap:8})}>{(j.photos||[]).map((p,i)=>(
        <div key={i} style={{position:"relative"}}>
          ...
          <button onClick={()=>setJ((x)=>({...x,photos:x.photos.filter((_,k)=>k!==i)}))} style={{position:"absolute",top:-6,right:-6,background:"#c05c4d",color:"#fff",borderRadius:999,width:20,height:20,fontSize:12,border:"none",cursor:"pointer"}}>✕</button>
        </div>))}
      </div>
    ...
    <div>
      ...
      {jobs.length===0?(
        <div style={{textAlign:"center",padding:"40px 16px",background:CARD,border:`1px dashed ${LINE}`,borderRadius:12}}>
          ...
          <button onClick={onGoWorkshop} style={{marginTop:12,background:OAK,color:ESP,padding:"9px 16px",borderRadius:8,fontWeight:700,border:"none",cursor:"pointer"}}>Go to Workshop</button>
        </div>
      ):(
        <div style={grid3()}>{jobs.map((j)=>{const cover=j.photos&&j.photos[0];return(
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:40,padding:16}}>
      <div onClick={(e)=>e.stopPropagation()} style={{background:CARD,borderRadius:12,padding:20,width:"100%",maxWidth:420}}>
        ...
        <div style={{...col({gap:12})}}>
          ...
          <Field label="Phone"><input style={inp} value={s.phone} onChange={(e)=>up("phone",e.target.value)}/></Field>
        </div>
        <div style={{...row({justifyContent:"space-between"}),marginTop:16}}>
          <button onClick={()=>{if(window.confirm("Delete ALL jobs? Cannot be undone."))onClearAll();}} style={{color:"#c05c4d",fontSize:13,background:"none",border:"none",cursor:"pointer"}}>Clear all data</button>
          <button onClick={()=>{onSave(s);onClose();}} style={{padding:"9px 18px",borderRadius:8,background:ESP,color:CREAM,fontWeight:600,border:"none",cursor:"pointer"}}>Save</button>
        </div>
      </div>
    </div>
  );
